...
export default function Login() {
  ...
  const [recoveryStep, setRecoveryStep] = useState(1); // 1 = email, 2 = token, 3 = nova senha
  ...
  const redirectUser = async (user: any) => {
    ...
    if (tipoUsuario === 'Professor') {
      console.log('Redirecionando professor para /home-professor');
      ...
      try {
        ...
        if (alunoData.success && alunoData.aluno && alunoData.aluno.teste_realizado) {
          console.log('Aluno já fez o teste - redirecionando para /home-aluno');
          window.location.href = '/home-aluno';
        } else {
          console.log('Aluno precisa fazer o teste - redirecionando para /home');
    ...
    if (recoveryStep === 1) {
      ...
      try {
        ...
        if (result.success) {
          ...
          setRecoveryEmail(emailToUse); // Salva o email para as próximas etapas
          setRecoveryStep(2);
        } else {
          showNotification(`Erro: ${result.message}`, 'error');
        }
